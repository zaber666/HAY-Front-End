...
def psychiatrist_details(_id):
    psychiatrist, person = db.session.query(Psychiatrist, Person).filter(Psychiatrist.psychiatrist_id == Person.person_id).filter(Psychiatrist.psychiatrist_id == _id).first()
    ret = {}
    ret["id"] = _id
    ret["name"] = person.name
    ret["cell"] = person.cellphone
    ...
    if test_response_id is None:
        ...
        return jsonify({"verified_reports": [{"reportId" : x[0].test_result_id, "test_name" : x[1].name, 
                                            ...
                                            "manual_report" : x[0].manual_report, "systemScore": x[0].score,
                                             "suggestedPsychiatrists": sugg.get(x[0].test_result_id, []) } for x in test_result]})
    else:
        test_result = TestResult.query.filter_by(test_result_id=test_response_id).first()
        try:
